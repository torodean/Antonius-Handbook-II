...
\textbf{Changing directory} via terminal via the \textbf{cd}\index{cd} command.
\begin{lstlisting}
...
To find a file within a folder or its sub-folders, you can use the \textbf{find}\index{find} command.
\begin{lstlisting}
...
\textbf{Mount}\index{mount} and \textbf{unmount}\index{unmount} a partition
\begin{lstlisting}
...
To open a \textbf{pdf}\index{pdf} via terminal, most generic desktop environments support
\begin{lstlisting}
...
Create a new user using the \textbf{useradd}\index{useradd} command.
\begin{lstlisting}
...
Change the user in terminal using the \textbf{su}\index{su} command.
\begin{lstlisting}
su - <USERNAME>
\end{lstlisting}
...
To make a file executable, or change the permissions in general the \textbf{chmod}\index{chmod} command can be used and is typically used as follows.
\begin{lstlisting}
...
The \textbf{mkfifo}\index{mkfifo} command is used to create a new named pipe. A pipe is used to store the output of one program to be used in another.
\begin{lstlisting}
...
\subsection{emacs}
\textbf{Emacs}\index{emacs} is a powerful text editor. You can open a document through emacs using the following. The ``-nw" flag indicates no GUI window should open (open in terminal).
\begin{lstlisting}
emacs doc.txt -nw
\end{lstlisting}
To toggle \textbf{read-only mode} use C-x C-q.
