...
The command-line utility used for querying and displaying logs from the systemd journal is \idx{journalctl}. The `-e' flag will start the log at the end (most recent) logged messages, which is often useful.
\begin{lstlisting}
sudo journalctl -xe
\end{lstlisting}
