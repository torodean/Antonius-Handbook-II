...
To access the Windows 7 "\idx{God Mode}" which is essentially a collection of administrator and troubleshooting features, create a folder with the following name:
\begin{lstlisting}
...
To view and modify \idx{mouse} settings, such as sensitivity or speed, you can open a run window and enter
\begin{lstlisting}[style=terminalstyle]
main.cpl		# Opens the mouse settings.
\end{lstlisting}

The \idx{System File Checker} is a built-in tool that scans for and restores corrupted system files. You can run it from the Command Prompt with administrative privileges.
\begin{lstlisting}[style=terminalstyle]
sfc /scannow
\end{lstlisting}